...
- **Missing `Evidence Workspace` section**: Add the section with `Schema version: 1.0`, `Claims path: claims.json`, `Weak Points path: weak-points.json`, `Active application ID:` and `Last validation:`. Do not create application or Claim facts from existing prose; paths may point to empty template collections until the candidate activates evidence-aware coaching.
- **Story Details missing `Project ID` / `Claim IDs`**: Add blank metadata fields when a story is next improved. Do not attempt automatic semantic linking.
- **Evidence JSON version check**: Validate evidence files independently of Markdown migration. Version `1.0` is supported. Unknown major versions are read-only blockers and must not be silently rewritten.
